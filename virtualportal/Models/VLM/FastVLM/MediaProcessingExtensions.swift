...
enum MediaProcessingExtensions {
    ...
    #if DEBUG
            print("[MediaProcessingExtensions.resampleBicubic] filter.outputImage is nil; returning original image")
    #endif
            return image
        }
        ...
        -> MLXArray
    {
        ...
        data.withUnsafeMutableBytes { ptr in
            guard let baseAddress = ptr.baseAddress else {
                #if DEBUG
                print("[MediaProcessingExtensions.asPlanarMLXArray] data baseAddress is nil")
                #endif
                failed = true
                return
            }
            ...
            var rgbaSrc = vImage_Buffer(
                data: baseAddress, height: vh, width: vw, rowBytes: bytesPerRow)
            var rgbDest = vImage_Buffer(
                data: baseAddress, height: vh, width: vw, rowBytes: rgbBytesPerRow)
            ...
            planarData.withUnsafeMutableBytes { planarPtr in
                guard let pBase = planarPtr.baseAddress else {
                    #if DEBUG
                    print("[MediaProcessingExtensions.asPlanarMLXArray] planarData baseAddress is nil")
                    #endif
                    failed = true
                    return
                }
                ...
                var rDest = vImage_Buffer(
                    data: pBase.advanced(by: 0 * planeBytesPerRow * h), height: vh,
                    width: vw, rowBytes: planeBytesPerRow)
                var gDest = vImage_Buffer(
                    data: pBase.advanced(by: 1 * planeBytesPerRow * h), height: vh,
                    width: vw, rowBytes: planeBytesPerRow)
                var bDest = vImage_Buffer(
                    data: pBase.advanced(by: 2 * planeBytesPerRow * h), height: vh,
                    width: vw, rowBytes: planeBytesPerRow)
        ...
        if failed {
            #if DEBUG
            print("[MediaProcessingExtensions.asPlanarMLXArray] Failed to generate planarData; returning zeros")
            #endif
            let zeros = [Float32](repeating: 0.0, count: 1 * 3 * h * w)
            return zeros.withUnsafeBytes { ptr in
                MLXArray(ptr, [1, 3, h, w], type: Float32.self)
            }
        }
